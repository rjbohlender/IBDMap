...
const std::vector<std::string> Binomial::links{"logit", "probit", "cloglog",
                                               "Cauchit", "Log"};
...
arma::vec Binomial::linkinv(const arma::mat &X,
                            const arma::vec &beta) noexcept {
  switch (linkid) {
  case Binomial::LinkID::Logit:
...
arma::vec Binomial::dev_resids(const arma::vec &y, const arma::vec &mu,
                               const arma::vec &weight) noexcept {
...
void Binomial::initialize(arma::vec &y, arma::vec &n, arma::vec &mu,
                          arma::vec &weight) noexcept {
  n = arma::vec(y.n_elem, arma::fill::ones);
  mu = (weight % y + 0.5) / (weight + 1);
}

double Binomial::aic(arma::vec &y, arma::vec &n, arma::vec &mu,
                     arma::vec &weight, double dev, double rank) noexcept {
  ...
  for (int i = 0; i < y.n_elem; i++) {
    ...
    aic_val +=
        -2 * std::log(boost::math::pdf(binom, boost::math::round(m(i) * y(i))));
  }
