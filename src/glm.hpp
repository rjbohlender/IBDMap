...
template <typename LinkT> struct GLM {
  LinkT link;
  ...
  GLM(arma::mat &X, arma::vec &Y, LinkT &link, const std::string& optimizer) {
    ...
    if (optimizer == "irls") {
      beta_ = irls(X, Y);
    } else if (optimizer == "irls_svdnewton") {
      beta_ = irls_svdnewton(X, Y);
    } else if (optimizer == "irls_qr") {
      beta_ = irls_qr(X, Y);
    } else if (optimizer == "irls_qr_R") {
      beta_ = irls_qr_R(X, Y);
  ...
  GLM(arma::mat &X, arma::vec &Y, LinkT &link, arma::vec initial_beta,
    ...
    if (initial_beta.n_rows < X.n_cols) {
      initial_beta.insert_rows(initial_beta.n_rows,
                               X.n_cols -
                                   initial_beta.n_rows); // Zeroed by default
    }
...
auto GLM<LinkT>::irls_svdnewton(arma::mat &X, arma::colvec &Y) -> arma::vec {
  ...
  do {
    ...
    // } while(iter < max_iter && std::abs(dev_ - devold) / (0.1 +
    // std::abs(dev_)) > tol);
...
auto GLM<LinkT>::svd_subset(arma::mat &X) -> arma::uvec {
  ...
  arma::rowvec sds =
      arma::sqrt(arma::pow(arma::sum(A.each_row() - arma::mean(A), 0), 2));
...
auto GLM<LinkT>::irls_qr(arma::mat &X, arma::colvec &Y) -> arma::vec {
  ...
  do {
    ...
    dev_ = arma::accu(link.dev_resids(Y, mu_, weights_));
           ...
           std::abs(dev_ - devold) / (0.1 + std::abs(dev_)) > tol);
      ...
      while (!std::isfinite(dev_)) {
        if (i >= max_iter) {
          throw(std::runtime_error(
              "Unable to correct step size for divergent IRLS step."));
        }
        beta_ = (beta_ + xold) / 2.;
        eta_ = X * beta_;
        mu_ = link.linkinv(eta_);
