   ...
   */
   ...
   if (colors2 < 3) {  
      ...
      if (nn < 5000 * colors2) {
         ...
         CMultiWalleniusNCHypergeometric wmnc(n, &osource[0], &oweights[0], colors2);

         wmnc.mean(&var[0]); // calculate mean
...
int32_t * destination, int32_t * source, double * weights, int32_t n, int colors) {
   ...
   std::array<double, MAXCOLORS> rweights;         // reciprocal weights
   std::array<int32_t, MAXCOLORS> sample;          // balls sampled
   double w;                           // weight
   ...
   MultiWalleniusNCHyp(&sample[0], source, &rweights[0], N - n, colors);
...
int32_t * source, double * weights, int32_t n, int colors) {
   ...
   */
   ...
   std::vector<int> order2(colors);              // corresponding index into osource when equal weights pooled together
   ...
   std::vector<int32_t> osample(colors);         // balls sampled, sorted by weight
   ...
   if (n < 0 || colors < 0 || colors > MAXCOLORS)
      FatalError("Parameter out of range in function MultiFishersNCHyp");
   if (colors == 0) return;
   if (n == 0) {
      for (i=0; i<colors; i++) destination[i] = 0; return;
   }
   ...
   if (n >= msum) {
      if (n > msum)
         FatalError("Taking more items than there are in function MultiFishersNCHyp");
      for (i = 0; i < colors; i++) {c = order1[i];  destination[c] = source[c];}
      return;
   }
   ...
   if (colors2 > 2) {
      ...
      // calculate variance
      CMultiFishersNCHypergeometric(n, &osource[0], &oweights[0], colors2).variance(&var[0]);
