...
// random number generator classes StochasticLib1, 2, and 3.
      ...
      #include "sfmt.h"
      #define STOC_BASE CRandomSFMT      // Binary library SFMT generator
   #endif
#endif
...
protected:
   ...
   // data
   std::array<int32_t, MAXCOLORS> xi;                                  // x vector to calculate probability of
   std::array<int32_t, MAXCOLORS> xm;                                  // rounded approximate mean of x[i]
   std::array<int32_t, MAXCOLORS> remaining;                           // number of balls of color > c in urn
   std::array<double, MAXCOLORS> sx;                                   // sum of x*f(x)
   std::array<double, MAXCOLORS> sxx;                                  // sum of x^2*f(x)
...
protected:
   ...
   double * odds;
   int colors;
   std::vector<double> logodds;                              // log odds
   ...
   std::vector<int32_t> xi;                                  // x vector to calculate probability of
   std::vector<int32_t> xm;                                  // rounded approximate mean of x[i]
   std::vector<int32_t> remaining;                           // number of balls of color > c in urn
   std::vector<double> sx;                                   // sum of x*f(x) or mean
   std::vector<double> sxx;                                  // sum of x^2*f(x) or variance
