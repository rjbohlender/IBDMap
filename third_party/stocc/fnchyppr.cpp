...
CMultiFishersNCHypergeometric::CMultiFishersNCHypergeometric(int32_t n_, int32_t * m_, double * odds_, int colors_, double accuracy_) {
   ...
   logodds = std::vector<double>(colors_);
   xi = std::vector<int32_t>(colors_);                                  // x vector to calculate probability of
   xm = std::vector<int32_t>(colors_);                                  // rounded approximate mean of x[i]
   remaining = std::vector<int32_t>(colors_);                           // number of balls of color > c in urn
   sx = std::vector<double>(colors_);                                   // sum of x*f(x) or mean
   sxx = std::vector<double>(colors_);                                  // sum of x^2*f(x) or variance
...
double CMultiFishersNCHypergeometric::loop(int32_t n, int c) {
   ...
   if (c < colors-1) {
      ...
      xi[c] = n;
      // sums and squaresums    
      s1 = exp(lng(&xi[0]));               // proportional function g(x)
