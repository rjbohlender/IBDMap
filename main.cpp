...
int main(int argc, char *argv[]) {
  ...
  app.add_option("-p,--pheno", params.pheno, "Path to file containing sample phenotype pairs. 1 for affected, 0 for unaffected, NA for sample to be skipped. Header line is required.")->required();
  app.add_option("-g,--gmap", params.gmap, "Recombination map files. Assumed to be three columns, position, chromosome, cM. Header line is required.")->required();
  ...
  app.add_option("--lower_bound", params.lower_bound, "If set, establishes a lower bound on the number of pairs that must be present at breakpoint for it to be included.");
  app.add_option("-m,--min_dist", params.min_dist, "Sets the minimum genetic distance between sites. The parser will automatically skip breakpoints that are closer than the given distance. Default value of 0.0 cM.")->default_val(0.0);
  app.add_option("--cm", params.cM, "Sets the minimum segment length for segments to be included. The parser will automatically skip segments that are smaller than the given length.");
  app.add_option("--af", params.AF, "Sets the maximum allele frequency for segments to be included. The parser will automatically skip segments that are more frequent than the given cutoff.");
  app.add_option("-r,--rsquared", params.rsquared, "Sets the maximum correlation between sites. The parser will automatically skip breakpoints that are closer than the given distance. Default value of 1.0.");
  app.add_option("--range",
				 params.range,
				 "Range of positions, separated by a comma. e.g., 100000,250000 will only analyze breakpoints with positions in that range, inclusive of the endpoints.")->delimiter(',')->expected(2);
  ...
  app.add_flag("--swap_pheno", params.swap, "Swap case-control status of individuals. Useful for examining problematic datasets.");
  ...
  std::cerr << "Input: " << params.input << std::endl;
  std::cerr << "Phenotypes: " << params.pheno << std::endl;
  std::cerr << "GMAP: " << params.gmap << std::endl;
  if(params.cov) {
	std::cerr << "COV: " << *params.cov << std::endl;
  }
  if(params.info) {
    std::cerr << "INFO: " << *params.info << std::endl;
  }
